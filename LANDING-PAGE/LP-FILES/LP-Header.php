<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Landing Page</title>
  <link rel="stylesheet" href="../LP-CSS/LP-Header.css">
  <! --==========START OF BOOTSTRAP LINK==========-->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-sRIl4kxILFvY47J16cr9ZwB07vP4J8+LH7qKQnuqkuIAvNWLzeN8tE5YBujZqJLB" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js" integrity="sha384-FKyoEForCGlyvwx9Hj09JcYn3nv7wiPVlz7YYwJrWVcXK/BmnVDxM+D2scQbITxI" crossorigin="anonymous"></script>
    <! --==========END OF BOOTSTRAP LINK==========-->
  <link rel="icon" href="../../INCLUDES/LP-IMAGES/awegreen-logo.png" type="image/png"
        </head>

<body>
  <nav class="navbar navbar-expand-lg fixed-top navbar-light bg-white" id="mainNavbar">
    <div class="container-fluid mx-5">

      <a href="#" class="fw-bold text-decoration-none flex gap-1 fs-20">
        <img class="awegreen-logo" src="../../INCLUDES/LP-IMAGES/awegreen-logo.png" alt="A We Green Logo" />
        <span class="green-text ">A We Green Enterprise</span>
      </a>

      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
      </button>

      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav ms-auto mb-2 mb-lg-0 gap-2 me-2">
          <li class="nav-item">
            <a class="nav-link fs-14 fw-semibold" aria-current="page" href="#hero">Home</a>
          </li>
          <li class="nav-item">
            <a class="nav-link fs-14 fw-semibold" href="#about-us">About Us</a>
          </li>
          <li class="nav-item">
            <a class="nav-link fs-14 fw-semibold" href="#services">Services</a>
          </li>
          <li class="nav-item">
            <a class="nav-link fs-14 fw-semibold" href="#projects">Projects</a>
          </li>
          <li class="nav-item">
            <a class="nav-link fs-14 fw-semibold" href="#choose-us">Choose Us</a>
          </li>
        </ul>
        <a href="../../LOGS/LOGS-FILES/login.php" class="btn green-bg text-white fs-14 login-btn" type="button">Login</a>
      </div>
    </div>
  </nav>
</body>

</html>
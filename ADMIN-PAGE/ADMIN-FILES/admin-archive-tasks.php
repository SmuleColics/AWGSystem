...
// Get statistics for ARCHIVED tasks only
$stats = [
  'total_tasks' => 0,
  'pending' => 0,
  'in_progress' => 0,
...
$result = mysqli_query($conn, "SELECT COUNT(*) as count FROM tasks WHERE is_archived = 1");
if ($result) {
  $row = mysqli_fetch_assoc($result);
  $stats['total_tasks'] = $row['count'];
}

$result = mysqli_query($conn, "SELECT COUNT(*) as count FROM tasks WHERE status = 'Pending' AND is_archived = 1");
if ($result) {
  $row = mysqli_fetch_assoc($result);
  $stats['pending'] = $row['count'];
}

$result = mysqli_query($conn, "SELECT COUNT(*) as count FROM tasks WHERE status = 'In Progress' AND is_archived = 1");
if ($result) {
  $row = mysqli_fetch_assoc($result);
  $stats['in_progress'] = $row['count'];
}

$result = mysqli_query($conn, "SELECT COUNT(*) as count FROM tasks WHERE status = 'Completed' AND is_archived = 1");
if ($result) {
  $row = mysqli_fetch_assoc($result);
  $stats['completed'] = $row['count'];
}

// Get all ARCHIVED tasks only
$tasks = [];
$sql = "SELECT * FROM tasks WHERE is_archived = 1 ORDER BY due_date ASC";
        ...
        <div class="tasks-container rounded-3 bg-white">
          ...
          <?php if (empty($tasks)): ?>
            <div class="text-center py-5">
              ...
              <p class="text-muted">All your tasks are active.</p>
            </div>
          <?php else: ?>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="alert alert-warning">
            ...
            This task is archived. Restore it to make changes.
          </div>
          ...
          <div class="mb-3">
            ...
            <textarea id="viewTaskDesc" class="form-control" rows="3" readonly></textarea>
          </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      $('.view-task').on('click', function() {
        ...
        $('#viewTaskDesc').val($(this).data('desc'));
